...
if __name__ == '__main__':
    ...
    for idx, data in enumerate(test_dl):
        ...
        with torch.no_grad():
            ...
            # save the padding image
            cv2.imwrite(f'preprocess/{img_name}', np.transpose(img_lq.squeeze().cpu().numpy()[[2,1,0],:,:], (1, 2, 0))*255) #RGB
            output = demo_UHD_fast(img_lq, model) # whole SR image, RGB
            
            preds = (output[:, :, :h_old*3, :w_old*3].clamp(0, 1) * 255).round() # 1 -> 255
            # [1,3,276,276]
        cv2.imwrite(f'result/{img_name}', np.transpose(preds.squeeze().cpu().numpy()[[2,1,0],:,:], (1, 2, 0))) #RGB
